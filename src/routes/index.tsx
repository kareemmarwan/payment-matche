...
export const Route = createFileRoute("/")({
  ...
  head: () => ({
    meta: [
      { title: "المطابقة الذكية للمدفوعات — تسوية التحويلات البنكية في ثوانٍ" },
      {
        name: "description",
        content:
          "قم برفع كشف الحساب البنكي وملفات مبيعات التجار للكشف تلقائيًا عن العملاء الذين دفعوا، والذين لم يدفعوا، والذين دفعوا مبالغ أقل أو أعلى. 100% داخل المتصفح.",
      },
    ],
  }),
});
...
function Index() {
  ...
  return (
    // تم إضافة dir="rtl" لضمان توافق اتجاه الصفحة بالكامل مع العربية
    <main className="min-h-screen pb-20" dir="rtl">
      ...
      <div className="mx-auto max-w-6xl space-y-10 px-4 py-8 sm:px-6">
        ...
        <section className="rounded-2xl border bg-card p-6 shadow-[var(--shadow-card)]">
          ...
          {/* تم تعديل الاتجاه هنا إلى اليسار ليناسب النمط العربي (المطابقة لـ justify-end في RTL تعني اليسار) */}
...
<div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
  ...
  dir="rtl"
>
  {/* الترتيب الآن طبيعي ومنطقي جداً، وسيبدأ من اليمين تماماً على الموبايل والكمبيوتر */}
  ...
  <TabsTrigger value="overpaid" className="whitespace-nowrap">أعلى ({stats.over})</TabsTrigger>
                ...
                <div className="relative w-full sm:w-72">
                  {/* تم تعديل كلاس الأيقونة ليصبح right-3 بدلاً من left-3 في الـ RTL */}
                  <Search className="absolute right-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" />
                  <Input
                    placeholder="ابحث بالاسم أو رقم الهاتف..."
                    value={query}
                    onChange={(e) => setQuery(e.target.value)}
                    className="pr-9 pl-3" // تم تغيير الـ Padding ليصبح pr-9 بدلاً من pl-9 لترك مساحة للأيقونة على اليمين
                  />
                </div>
              </div>
...
function EmptyState() {
  return (
    <section className="rounded-2xl border border-dashed bg-card/50 p-10 text-center">
      ...
      {/* تم تغيير النص إلى محاذاة اليمين text-right لتتناسب مع العربية */}
